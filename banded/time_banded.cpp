 ...
 * nrhs = num of rhs
 */
double time_cholesky(int n, int nrhs){
	CholeskyBanded cb(n);
	Random rng;
	double *tmp = new double[n];
	for(int i=0; i < n-2; i++)
...
void print_cholesky_info(int n, int nrhs){
	cout<<setw(50)<<"Timing Banded Cholesky (l,u = 2) Solver"<<endl;
	cout<<setw(50)<<"reuses same factrization"<<endl;
	cout<<setw(50)<<"n: "<<n<<endl;
	if(nrhs > 0)
		cout<<setw(50)<<"nrhs: "<<nrhs<<endl;
	...
	cout<<setw(50)<<"nmlzed by n"<<endl;
	double nmlz = n;
	double cycles = time_cholesky(n, nrhs);
	cout<<setw(25)<<"cycles: "<<cycles/nmlz<<endl<<endl;
}
